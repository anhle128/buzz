...
fn github_merge_skips_merge_for_each_blocked_open_gate() {
    ...
    let mut pending_check = MergeFake::ready();
    pending_check.checks_status = 8;
    pending_check.checks = r#"[{"name":"desktop-ci","state":"PENDING","bucket":"pending","link":"https://github.com/acme/buzz/actions/runs/2"}]"#.to_string();
    cases.push((
        "pending required check exit 8",
        ready_view_value(),
        pending_check,
    ));
