...
**Tier-1 — compiled-in runtimes** (Goose, Claude Code, Codex, Buzz Agent, Hermes Agent): have auto-installers, auth probes, and first-class onboarding. Their IDs (`goose`, `claude`, `codex`, `buzz-agent`, `hermes`) are reserved and cannot be overridden. Hermes sessions receive `buzz-dev-mcp` so `buzz messages send` can sign as the agent.
