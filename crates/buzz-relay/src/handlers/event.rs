...
/// True when a relay-signed event carries `buzz:workflow` or `buzz:app`.
///
/// These messages are generated by the relay itself and must not re-enter
/// the workflow engine.
pub(crate) fn is_relay_generated_message(event: &Event, relay_pubkey: PublicKey) -> bool {
...
async fn dispatch_persistent_event_inner(
    ...
    // workflow or App messages (`buzz:workflow` / `buzz:app`).
    let skip_workflows =
        is_relay_generated_message(&stored_event.event, state.relay_keypair.public_key());
...
mod tests {
    ...
    #[test]
    fn is_relay_generated_message_matches_workflow_or_app_tags() {
        let relay = Keys::generate();
        let other = Keys::generate();
        let workflow = EventBuilder::new(Kind::from(KIND_STREAM_MESSAGE as u16), "hi")
            .tags([Tag::parse(["buzz:workflow", "true"]).unwrap()])
            .sign_with_keys(&relay)
            .unwrap();
        let app = EventBuilder::new(Kind::from(KIND_STREAM_MESSAGE as u16), "hi")
            .tags([Tag::parse(["buzz:app", "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"]).unwrap()])
            .sign_with_keys(&relay)
            .unwrap();
        let plain = EventBuilder::new(Kind::from(KIND_STREAM_MESSAGE as u16), "hi")
            .sign_with_keys(&relay)
            .unwrap();
        let spoofed = EventBuilder::new(Kind::from(KIND_STREAM_MESSAGE as u16), "hi")
            .tags([Tag::parse(["buzz:app", "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"]).unwrap()])
            .sign_with_keys(&other)
            .unwrap();
        assert!(super::is_relay_generated_message(
            &workflow,
            relay.public_key()
        ));
        assert!(super::is_relay_generated_message(&app, relay.public_key()));
        assert!(!super::is_relay_generated_message(
            &plain,
            relay.public_key()
        ));
