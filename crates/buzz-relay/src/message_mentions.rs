...
//! autocomplete pick; workflow and App callback paths have only free text, so
//! this reverse-parse *defines* the matching contract.

/// Resolves `@Name` mentions in relay-signed message text to the pubkeys of the
/// channel members they name, so the emitted kind:9 carries the `p` tags that
/// ACP agent-wake (`event_mentions_agent`) is gated on.
///
/// The client resolves mentions to `p` tags at compose time from an interactive
/// autocomplete pick; workflow and App callback paths have only free text, so
/// this reverse-parse *defines* the matching contract. It is deliberately
/// conservative to avoid waking the wrong agent:
///
...
pub(crate) fn resolve_mention_pubkeys(text: &str, members: &[(String, String)]) -> Vec<String> {
